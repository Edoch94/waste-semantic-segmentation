...
class BottleNeck(nn.Module):
    ...
    def __init__(
        ...
        internal = self.output_channels // 4
        input_stride = 2 if downsampling else 1
        # First projection with 1x1 kernel (2x2 for downsampling)
        conv1x1_1 = nn.Conv2d(
            self.input_channels, internal, input_stride, input_stride, bias=False
        )
        ...
        if downsampling:
            ...
            spatial_conv = nn.Conv2d(
                self.input_channels, self.output_channels, 1, bias=False
            )
            ...
            conv = nn.Conv2d(
                ...
                internal,
                3,
                padding=dilation_rate,  # type: ignore
                dilation=dilation_rate,  # type: ignore
            )
        elif asymmetric:
            conv1 = nn.Conv2d(internal, internal, [5, 1], padding=(2, 0), bias=False)  # type: ignore
            conv2 = nn.Conv2d(internal, internal, [1, 5], padding=(0, 2))  # type: ignore
    ...
    def forward(self, input, pooling_indices=None):
        ...
        if self.downsampling:
            return output, indices  # type: ignore
        return output
...
class Encoder(nn.Module):
    def __init__(self, num_classes, img_size, only_encode=True):
        super().__init__()
    ...
    def forward(self, input):
        ...
        if self.state:
            output = F.upsample(output, self.img_size, None, 'bilinear')
...
class ENet(nn.Module):
    def __init__(self, cfg: DictConfig, only_encode=False):
        super().__init__()
        self.state = only_encode
